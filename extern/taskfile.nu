...
def "nu-complete tasks" [] {
  task -a | lines | skip | str trim | parse '* {task}:' | get task
}

export extern task [
  ...task: string@"nu-complete tasks" # Runs the specified task or lists all tasks if an unknown task name was specified.
  ...
  --interval(-I): duration            # Interval to watch for changes.
